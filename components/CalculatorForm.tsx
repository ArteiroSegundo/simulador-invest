  ...
  return (
    <div>
      ...
      <div className="flex items-center rounded-md border border-slate-300 bg-white shadow-sm focus-within:border-[#2C3FA5] focus-within:ring-1 focus-within:ring-[#2C3FA5]">
        <span className="pl-3 pr-2 text-slate-500 font-medium">R$</span>
        <input
          type="text"
          id={id}
          value={value}
          onChange={onChange}
          className="w-full p-2 border-0 bg-transparent focus:ring-0 outline-none text-slate-700"
          placeholder="0,00"
        />
      </div>
    </div>
...
export const CalculatorForm: React.FC<CalculatorFormProps> = ({ onCalculate, onClear }) => {
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg mb-10 border border-slate-100">
      <h2 className="text-2xl font-bold text-[#2C3FA5] mb-6 flex items-center gap-2">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="flex">
              <span className="inline-flex items-center px-3 rounded-l-md border border-r-0 border-slate-300 bg-slate-50 text-slate-500 font-medium">%</span>
              <input
                type="text"
                id="interestRate"
                value={interestRate}
                onChange={(e) => setInterestRate(e.target.value)}
                className="flex-1 min-w-0 block w-full px-3 py-2 border-t border-b border-slate-300 bg-white focus:outline-none focus:ring-1 focus:ring-[#2C3FA5] focus:border-[#2C3FA5] sm:text-sm text-slate-700"
                placeholder="8"
              />
              <select
                value={rateType}
                onChange={(e) => setRateType(e.target.value as 'monthly' | 'yearly')}
                className="rounded-r-md border border-l-0 border-slate-300 bg-slate-50 text-slate-700 focus:ring-[#2C3FA5] focus:border-[#2C3FA5] cursor-pointer"
              >
          ...
          <div>
            ...
            <div className="flex">
              <input
                ...
                className="flex-1 min-w-0 block w-full px-3 py-2 rounded-l-md border border-slate-300 bg-white focus:outline-none focus:ring-1 focus:ring-[#2C3FA5] focus:border-[#2C3FA5] sm:text-sm text-slate-700"
                placeholder="10"
              />
              <select
                value={periodType}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-4 gap-4">
          <button
            type="submit"
            className="w-full sm:w-auto bg-[#2C3FA5] text-white font-bold py-3 px-10 rounded-lg hover:bg-[#243387] transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2C3FA5] shadow-md hover:shadow-lg active:scale-95"
          >
            Calcular
          </button>
          <button 
            ...
            className="text-sm text-slate-500 hover:text-[#2C3FA5] transition-colors font-medium underline underline-offset-4"
          >
            Limpar campos
          </button>
        </div>
